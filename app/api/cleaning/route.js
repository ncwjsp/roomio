...
export async function GET(req) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: "Failed to fetch cleaning requests", error }), {
      status: 500,
    });
  }
}
...
export async function POST(req) {
  ...
  try {
    ...
    const { roomNumber, building, floor, name, date, status, assignedTo } = body;

    if (!roomNumber || !building || !floor || !name || !date || !status || !assignedTo) {
    ...
    return new Response(JSON.stringify({ message: "Failed to create cleaning request", error }), {
      status: 500,
    });
  }
}
...
export async function PUT(req) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: "Failed to update cleaning request", error }), {
      status: 500,
    });
  }
}
...
export async function DELETE(req) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: "Failed to delete cleaning requests", error }), {
      status: 500,
    });
  }
}
