...
export default function TasksPage() {
  const [tasksToday, setTasksToday] = useState([
    {
      ...
      status: "incomplete",
      cost: 200,
      details: "Change the light bulb in the room.",
      defaultRate: 100,
      options: [
  ...
  const [tasksHistory, setTasksHistory] = useState([
    {
      ...
      status: "completed",
      cost: 500,
      details: "Cleaned and maintained the AC unit.",
    },
  ]);
  ...
  const handleMarkAsDone = () => {
    if (selectedTask) {
      const updatedTasks = tasksToday.filter(
  ...
  const handleOptionChange = (e) => {
    const option = selectedTask.options.find(
          ...
          <div className="bg-gray-100 p-5 rounded-lg shadow mb-5">
            ...
            <div className="mt-5">
              ...
              {selectedTask.selectedOption && (
                <p className="text-gray-600">
                  Added Cost ({selectedTask.selectedOption.name}):{" "}
                  {selectedTask.selectedOption.cost} Bath
                </p>
              )}
              <p className="font-bold text-gray-800">
                Total Cost: {calculateTotalCost()} Bath
              </p>
            </div>
          </div>
              ...
              {tasksToday.map((task) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-gray-600">
                      Room: {task.room} | Building: {task.building}
                    </p>
                  </div>
