    ...
    <div>
      <div className="grid grid-cols-4 gap-4 mb-8">
        {overviewStats.map((stat, index) => (
          <div key={index} className="p-4 bg-white rounded-[10px] shadow flex justify-between items-center">
            <div>
              <h3 className="text-sm text-gray-500">{stat.title}</h3>
              <p className="text-2xl font-bold">{stat.value}</p>
            </div>
            <div className={`w-10 h-10 flex items-center justify-center rounded-[10px] ${stat.iconBg}`}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <PieChartComponent data={overduePaymentPieData} title="Overdue Payments" />
        <PieChartComponent data={roomVacancyPieData} title="Room Vacancy" />
      </div>

      <PieChartComponent data={maintenancePieData} title="Maintenance & Cleaning" />
    </div>
  );
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {incomeSummaryCards.map((card, index) => (
            ...
            >
              ...
              <div className={`w-10 h-10 flex items-center justify-center rounded-[10px] ${card.iconBg}`}>
        ...
        <div>
          <div className="flex items-center mb-4">
            <button
              ...
              className={`px-4 py-2 ${view === "Monthly" ? "bg-[#898F63] text-white" : "bg-white text-black"} rounded-[10px] mr-4`}
            >
              Monthly
            </button>
            <button
              onClick={() => setView("Yearly")}
              className={`px-4 py-2 ${view === "Yearly" ? "bg-[#898F63] text-white" : "bg-white text-black"} rounded-[10px]`}
            >
              Yearly
            </button>
          </div>
          ...
          {selectedCard === "revenue" && (
            <LineChartComponent
              data={view === "Monthly" ? monthlyRevenueExpenseData : yearlyRevenueExpenseData.filter((data) => data.year === selectedYear)}
              title="Revenue Graph"
          ...
          {selectedCard === "net-profit" && (
            <LineChartComponent
              ...
              xKey={view === "Monthly" ? "month" : "year"}
              yKeys={[{ dataKey: "revenue", color: "#4AD991" }, { dataKey: "expense", color: "#F30505" }]}
            />
          )}
          {selectedCard === "electricity" && (
            <PieChartComponent data={electricityUsageData} title="Electricity Usage" />
          )}
          {selectedCard === "water" && (
          ...
          <button className="px-4 py-2 bg-[#898F63] text-white rounded-[10px] mt-4" onClick={() => setSelectedCard(null)}>
            Back
          </button>
        </div>
      )}
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center rounded-[10px] overflow-hidden">
            <button
              ...
              className={`px-6 py-2 ${tab === "Overview" ? "bg-[#898F63] text-white" : "bg-white text-black"}`}
            >
              Overview
            </button>
            <button
              onClick={() => setTab("Income Summary")}
              className={`px-6 py-2 ${tab === "Income Summary" ? "bg-[#898F63] text-white" : "bg-white text-black"}`}
            >
              Income Summary
            </button>
          </div>
